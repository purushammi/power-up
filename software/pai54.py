...
class software(object):
    ...
    def __init__(self, eval_ver=False, non_int=False):
        ...
        try:
            self.pkgs = yaml.load(open(GEN_SOFTWARE_PATH + 'pkg-lists54.yml'))
        except IOError:
            self.log.error('Error opening the pkg lists file (pkg-lists54.yml)')
            sys.exit('Exit due to critical error')

        if self.eval_ver:
            try:
                self.sw_vars = yaml.load(open(GEN_SOFTWARE_PATH + 'software-vars-eval54.yml'))
            except IOError:
                # if no eval vars file exist, see if the license var file exists
                # and start with that
                try:
                    self.sw_vars = yaml.load(open(GEN_SOFTWARE_PATH + 'software-vars54.yml'))
                except IOError:
        ...
        if self.eval_ver:
            ...
            try:
                temp = yaml.load(open(GEN_SOFTWARE_PATH + 'software-vars54.yml'))
            ...
            try:
                temp = yaml.load(open(GEN_SOFTWARE_PATH + 'software-vars-eval54.yml'))
        ...
        # expression of .*
        try:
            file_lists = yaml.load(open(GEN_SOFTWARE_PATH + 'file-lists54.yml'))
        except IOError:
    ...
    def README(self):
        ...
        text = ('\nThis module installs the PowerAI Enterprise software '
                ...
                'For installation status: pup software --status pai54\n'
                'To redisplay this README: pup software --README pai54\n\n'
        ...
        if self.eval_ver:
            cmd = ('{} -i {} {}init_clients.yml --extra-vars "@{}" '
                   .format(get_ansible_playbook_path(),
                           ...
                           GEN_SOFTWARE_PATH,
                           GEN_SOFTWARE_PATH + "software-vars-eval54.yml"))
        else:
            cmd = ('{} -i {} {}init_clients.yml --extra-vars "@{}" '
                   .format(get_ansible_playbook_path(),
                           self.sw_vars['ansible_inventory'],
                           GEN_SOFTWARE_PATH,
                           GEN_SOFTWARE_PATH + "software-vars54.yml"))
        prompt_msg = ""
    ...
    def _validate_ansible_become_pass(self, ansible_become_pass):
        ...
        if ansible_become_pass is not None:
            ...
            cmd += f'--extra-vars "@{GEN_SOFTWARE_PATH}software-vars54.yml" '
        else:
            cmd += ' --ask-become-pass '
        resp, err, rc = sub_proc_exec(cmd, shell=True)
        log.debug(f"cmd: {cmd}\nresp: {resp}\nerr: {err}\nrc: {rc}")
    ...
    def _run_ansible_tasks(self, tasks_path, extra_args=''):
        ...
        if self.eval_ver:
            cmd = (f'{get_ansible_playbook_path()} -i '
                   ...
                   f'--extra-vars "@{GEN_SOFTWARE_PATH}software-vars-eval54.yml" '
                   f'{extra_args}')
        else:
            cmd = (f'{get_ansible_playbook_path()} -i '
                   f'{self.sw_vars["ansible_inventory"]} '
                   f'{GEN_SOFTWARE_PATH}{self.my_name}_ansible/run.yml '
                   f'--extra-vars "task_file={GEN_SOFTWARE_PATH}{tasks_path}" '
                   f'--extra-vars "@{GEN_SOFTWARE_PATH}software-vars54.yml" '
                   f'{extra_args}')
        run = True
        while run:
